...
static struct udp_pcb *dhcps_netif_pcb[ESP_IF_MAX];
...
{
    esp_interface_t esp_if = tcpip_adapter_get_esp_if(netif);

    if (esp_if == ESP_IF_MAX) {
        printf("dhcps_start: Could not associate ESP IF with netif=%p\n", netif);
        return;
    }

    if (dhcps_netif_pcb[esp_if] != NULL) {
        udp_remove(dhcps_netif_pcb[esp_if]);
    }
    ...
    dhcps_netif_pcb[esp_if] = pcb_dhcps;
...
void dhcps_stop(struct netif *netif)
{
    ...
    if (netif == NULL) {
        printf("dhcps_stop: netif == NULL\n");
        return;
    }

    if (esp_if == ESP_IF_MAX) {
        printf("dhcps_stop: Could not associate ESP IF with netif=%p\n", netif);
        return;
    }

    if (dhcps_netif_pcb[esp_if] != NULL) {
        udp_disconnect(dhcps_netif_pcb[esp_if]);
        udp_remove(dhcps_netif_pcb[esp_if]);
        dhcps_netif_pcb[esp_if] = NULL;
    }
